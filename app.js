...
function cardSubtitle(card){ return state.lang === 'en' ? (card.subtitleEn || card.subtitle || card.vietnameseName || '') : (card.subtitle || card.englishName || ''); }
const KEYWORD_EN = {
...
function renderToc(){ els.mainToc.innerHTML = state.mainCards.map(card => '<button type="button" data-card-id="'+card.uid+'"><span>'+escapeHtml(card.displayNumber)+'</span>'+escapeHtml(cardTitle(card))+'</button>').join(''); els.planetaryToc.innerHTML = state.planetaryCards.map(card => '<button type="button" data-card-id="'+card.uid+'"><span>'+escapeHtml(card.displayNumber)+'</span>'+escapeHtml(cardTitle(card))+'</button>').join(''); els.birthToc.innerHTML = state.birthCards.map(card => '<button type="button" data-card-id="'+card.uid+'"><span>'+escapeHtml(card.displayNumber)+'</span>'+escapeHtml(cardTitle(card))+'</button>').join(''); els.zodiacToc.innerHTML = state.zodiacCards.map(card => '<button type="button" data-card-id="'+card.uid+'"><span>'+escapeHtml(card.displayNumber)+'</span>'+escapeHtml(cardTitle(card))+'</button>').join(''); }
function cardTile(card){ return '<button class="card-tile" type="button" data-card-id="'+card.uid+'"><span class="card-image-wrap"><img src="'+(card.image || buildCardImage(card))+'" alt="'+escapeHtml(card.displayNumber+' - '+cardTitle(card))+'" loading="lazy"></span><span class="card-meta"><span class="card-number">'+escapeHtml(card.displayNumber)+'</span><span class="card-name">'+escapeHtml(cardTitle(card))+'</span><span class="card-subtitle">'+escapeHtml(cardSubtitle(card))+'</span></span></button>'; }
...
function renderModal(card){
  ...
  els.modalImage.alt = card.displayNumber+' - '+cardTitle(card);
  els.modalNumber.textContent = card.displayNumber;
  els.modalTitle.textContent = cardTitle(card);
  els.modalSubtitle.textContent = cardSubtitle(card);
  els.modalEnergy.innerHTML = makeTags((card.energy || []).map(energyLabel));
